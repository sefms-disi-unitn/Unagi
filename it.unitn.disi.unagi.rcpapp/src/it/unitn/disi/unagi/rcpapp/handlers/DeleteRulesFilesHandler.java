...
/**
 * Handler for the "Delete Rules File" command.
 * 
 * @author Vitor E. Silva Souza (vitorsouza@gmail.com)
 * @version 1.0
 */
public class DeleteRulesFilesHandler extends AbstractRulesFilesHandler {
	...
	public void execute(@Named(IServiceConstants.ACTIVE_SHELL) Shell activeShell, ESelectionService selectionService) {
		LogUtil.log.debug("Executing \"Compile Rules File(s)\" command."); //$NON-NLS-1$

		// Confirms the deletion of the files.
		Set<IFile> files = retrieveMultipleSelectedRules(selectionService);
		int size = files.size();
		if (size > 0) {
	...
	protected void doExecute(IProgressMonitor monitor, IFile file) throws UnagiException {
		...
		manageModelsService.deleteRulesFile(monitor, file);
	}
}
